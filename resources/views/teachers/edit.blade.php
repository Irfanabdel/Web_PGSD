<x-app-layout title="Edit Modul Guru">
    <div class="p-6 sm:ml-64 pt-8">
        <div class="bg-white shadow-md rounded-lg p-6 mb-6">
            <h1 class="text-2xl font-extrabold tracking-tight leading-tight text-gray-900 md:text-4xl lg:text-4xl mb-6">Edit Modul</h1>
            <form action="{{ route('teachers.update', $teacher->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="mb-4">
                    ...
                    <select name="theme_id" id="theme_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-yellow-400 focus:border-yellow-400 block w-full p-2.5" required>
                        ...
                        <option value="{{ $theme->id }}" 
                                ...
                                {{ $teacher->theme_id == $theme->id ? 'selected' : '' }}>
                            {{ $theme->title }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="user_kelas" id="user_kelas" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-yellow-400 focus:border-yellow-400 block w-full p-2.5" required>
                        ...
                        <option value="{{ $kelasItem }}" 
                                {{ $teacher->user_kelas == $kelasItem ? 'selected' : '' }}>
                            {{ $kelasItem }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-yellow-400 focus:border-yellow-400 block w-full p-2.5" required>{{ $teacher->description }}</textarea>
                </div>
                ...
                <div id="teacher-files-section" class="p-4 border border-gray-300 rounded-lg mb-6">
                    ...
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">File saat ini: 
                            <a href="{{ Storage::url($teacher->files) }}" class="text-blue-600 hover:underline" target="_blank">
                                {{ basename($teacher->files) }}
                            </a>
                        </p>
                    </div>
                    <p class="mt-2 text-sm text-gray-500">Anda dapat mengunggah file baru untuk mengganti file saat ini. Jenis file yang diterima: PDF, DOC, DOCX, PPT, PPTX</p>
                    @error('files')
